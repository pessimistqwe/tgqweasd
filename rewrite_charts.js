...
const filepath = 'frontend/script.js';
let content = fs.readFileSync(filepath, 'utf-8');

if (content.includes('LightweightCharts') && !content.includes('Chart.js')) {
...
const startStr = "// Chart rendering using Chart.js (Polymarket style) with gradient";
const endStr = "function updateChartPriceDisplay(";
...
const newCode = `// Chart rendering using Lightweight Charts
let eventChart = null;
...
let finalContent = content.slice(0, startIdx) + newCode + "\n" + content.slice(endIdx);

const wsStartStr = "function connectBinanceWebSocket(";
const wsEndStr = "// ==================== UTILITY FUNCTIONS ====================";

const startWs = finalContent.indexOf(wsStartStr);
const endWs = finalContent.indexOf(wsEndStr);

const wsCode = `function connectBinanceWebSocket(symbol) {
    if (binanceWebSocket) {
        binanceWebSocket.close();
        binanceWebSocket = null;
    }
...
finalContent = finalContent.slice(0, startWs) + wsCode + "\n" + finalContent.slice(endWs);
