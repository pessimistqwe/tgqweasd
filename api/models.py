...
class Event(Base):
    ...
    id = Column(Integer, primary_key=True)
    polymarket_id = Column(String(255), unique=True)  # Optional: for Polymarket sync
    title = Column(String(500), nullable=False)
    description = Column(Text)
    options = Column(Text, nullable=False)  # JSON: ["Да","Нет"]
...
class UserPrediction(Base):
    ...
    # Bet status
    is_winner = Column(Boolean, nullable=True)  # NULL = not settled
